...
export function createVueProject(projectName) {
    console.log(chalk.magenta('🚀 Creating Vue project with TypeScript...'));
    console.log(chalk.cyan(`Category: Web`));
    console.log(chalk.cyan(`Technology: Vue`));
    ...
    const reservedWords = ['vue', 'vite', 'node', 'npm', 'yarn', 'package', 'module', 'test', 'build', 'dev', 'prod'];
    ...
    try {
        console.log(chalk.yellow('📦 Creating new Vue project...'));
        execSync(`npm create vue@latest ${projectName} --typescript -- --typescript --router --pinia --eslint --prettier`, { stdio: 'inherit' });
        ...
        console.log(chalk.green(`✅ Vue project "${projectName}" created successfully!`));
        ...
        console.error(chalk.red('❌ Error creating Vue project:'), error.message);
        process.exit(1);
    }
} 