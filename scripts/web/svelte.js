...
export function createSvelteProject(projectName) {
    console.log(chalk.magenta('🚀 Creating Svelte project with Vite and TypeScript...'));
    console.log(chalk.cyan(`Category: Web`));
    console.log(chalk.cyan(`Technology: Svelte`));
    ...
    const reservedWords = ['svelte', 'vite', 'node', 'npm', 'yarn', 'package', 'module', 'test', 'build', 'dev', 'prod'];
    ...
    try {
        console.log(chalk.yellow('📦 Creating new Svelte project with Vite...'));
        execSync(`npm create vite@latest ${projectName} -- --template svelte-ts`, { stdio: 'inherit' });
        ...
        console.log(chalk.green(`✅ Svelte project "${projectName}" created successfully!`));
        ...
        console.error(chalk.red('❌ Error creating Svelte project:'), error.message);
        process.exit(1);
    }
} 